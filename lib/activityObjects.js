...
var ActivityObject = Class.extend({
           ...
           if(err){ callback(err,"") } else {
              ...
              render.renderTemplate(
                 ...
                 {                                  // Second arg
                    ...
                    target: {
                        type: self.item.target.type
                       ,url: self.item.target.url
                    }
                 },
    ...
    ,init: function(){
        ...
        this.item.url = "/dev/null";
        this.item.target = { url:"",type:""}
...
var Comment = ActivityObject.extend({
    ...
    init: function(){
        this._super();
        if (!(this.item && this.item.content && this.item.subject && this.item.permalink && this.item.target)){
            throw {name : "Invalid Comment exception", error:"Comment needs a subject, a content, a target and a permalink"}
        } else {
            this.item.id = this.item.permalink;
            this.item.content = utils.Encoder.htmlEncode(this.item.content,true);
...
var Note = ActivityObject.extend({
    ...
    init: function(){
        this._super();
        if (!(this.item && this.item.content && this.item.permalink && this.item.target)){
            ...
            if(!this.item.target){
                this.item.target.url = this.item.permalink
                this.item.target.type = "http://activitystrea.ms/schema/1.0/note"
            }
        }
    },
    renderItem : function(callback){
...
var Status = ActivityObject.extend({
    ...
    init: function(){
        ...
        if (!(this.item && this.item.content && this.item.permalink)){
            ...
            if(!this.item.target){
                this.item.target.url = this.item.permalink;
                this.item.target.type = "http://activitystrea.ms/schema/1.0/status"
            }
